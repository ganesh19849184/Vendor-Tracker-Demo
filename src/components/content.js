...
function Content() {
    ...
    const columns = [
        ...
        {dataField:'doc', text: 'Date of Clouser',sort:true},
        {dataField:'doc_vmpm', text: 'DOC VM/PM',sort:true},
    ...
    const apiData = async e =>{
        ...
        result = await axios.get(apiUrl+`requests`);
     ...
     allRequest.map((item, i) => {
       ...
       {   
        ...
        arCurrent.edit =  <Link to={`/editRequest/${item.id}`}> <Icon name = "edit" tooltip = "Edit" theme = "light" size = "small">  </Icon>  </Link>;
        arCurrent.delete =  <Icon name = "delete" tooltip = "Delete" theme = "light" size = "small" onClick={() => deleteRequest(item.id)}>  </Icon>    
        arCurrent.response = <Link to = {`/response/${item.id}`}> <Icon name = "comment" tooltip = "Response" theme = "light" size = "small">  </Icon></Link>; 
        if(item.id)
    ...
    const seen = new Set();
     ...
     const deleteRequest = async (id) => 
        ...
        swal({
            title: "Are you sure Delete Request?",
            icon: "warning",
            buttons: true,
            dangerMode: true,
          })
          .then((willDelete) => {
            if (willDelete) {
                 axios.delete(apiUrl+`requestDelete/${id}`);
            var newAllRequest = allRequest.filter((item) => {
                return item.id !== id;
            })
     ...
     return (
         <div>
              ...
              <RequestTabs/> 
                        <div>
                        <Link to={`/addRequest`}><Icon name = "add" tooltip = "Add" theme = "light" size = "medium"></Icon>
                        </Link> 
                        </div>
